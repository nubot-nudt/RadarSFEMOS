...
class vodDataset(Dataset):
    ...
    def __init__(self, args, root='/root/autodl-fs/preprocess_res/flow_smp/', partition='train', textio=None):
        ...
        self.samples = []
        self.extra_samples = []
        self.extra_percent = args.extra_percent
        self.clips_info = []
        ...
        for clip in self.clips:
            ...
            if clip[:6] == 'unanno':
                self.extra_samples.extend(samples)
        
        if self.partition == 'train_large':
            extra_len = len(self.extra_samples)
            self.samples.extend(self.extra_samples[:int(extra_len*self.extra_percent)])
    ...
    def __getitem__(self, index):
        ...
        # extract estimated optical flow info
