...
class SelfSupervisedLoss(Module):
    ...
    def __init__(self, w_sc=1, w_ss=1, w_rd=1,w_ar=0):
    ...
    def forward(self, pc1, pc2, pred_f, vel1):
        ...
        # err_i, err_ri = nn_errors(pc1, pc2, pred_f, Tr)
        # art_loss = artificial_label_loss(err_i, err_ri, L_cls)
        total_loss = self.w_sc * scloss + self.w_ss * ssloss + self.w_rd * rdloss# + self.w_ar*art_loss
        items={
            'Loss': total_loss.item(),
            'smoothnessLoss': ssloss.item(),
            'chamferLoss': scloss.item(),
            'veloLoss': rdloss.item(),
            #'art_loss': art_loss.item(),
        }
...
class RadarFlowLoss(Module):
    ...
    def forward(self, args, pc1, pc2, pred_f, vel1,gt_f=None, pre_trans=None, mseg_pre=None, gt_trans=None, \
        ...
        if args.model == 'raflow' or args.model == 'cmflow_b' or args.model=='pvraft':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            total_loss = self.w_self * self_sup_loss

        if args.model == 'cmflow_l':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            dyn_flow_loss = self.dyn_flow_loss(pred_f, gt_f, dyn_mask)
            motion_seg_loss = self.motion_seg_loss(mseg_pre, mseg_gt)
            items['superviseLoss'] = dyn_flow_loss.item() * self.w_dyn
            items['maskLoss'] = motion_seg_loss.item() * self.w_ms
            total_loss = self.w_self * self_sup_loss + self.w_dyn * dyn_flow_loss + self.w_ms * motion_seg_loss
        
        if args.model == 'cmflow_c':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            pc1_warp = pc1 + pred_f
            opt_flow_loss = self.opt_flow_loss(opt, radar_u, radar_v, pc1_warp, dyn_mask, args)
            items['opticalLoss'] = opt_flow_loss.item()
            total_loss = self.w_self * self_sup_loss + self.w_opt * opt_flow_loss

        if args.model == 'cmflow_lc':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            dyn_flow_loss = self.dyn_flow_loss(pred_f, gt_f, dyn_mask)
            pc1_warp = pc1 + pred_f
            opt_flow_loss = self.opt_flow_loss(opt, radar_u, radar_v, pc1_warp, pc1, args)
            items['opticalLoss'] = opt_flow_loss.item()
            items['superviseLoss'] = dyn_flow_loss.item()
            total_loss = self.w_self * self_sup_loss + self.w_opt * opt_flow_loss + self.w_dyn * dyn_flow_loss 


        if args.model == 'cmflow_o':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            ego_motion_loss = self.ego_motion_loss(pc1, pre_trans, gt_trans)
            motion_seg_loss = self.motion_seg_loss(mseg_pre, mseg_gt)
            items['egoLoss'] = ego_motion_loss.item() * self.w_em
            items['maskLoss'] = motion_seg_loss.item() * self.w_ms
            total_loss = self.w_self * self_sup_loss + self.w_em * ego_motion_loss + self.w_ms * motion_seg_loss
            if torch.any(torch.isnan(total_loss)):
                print('nan loss') 

        if args.model == 'cmflow_ol':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            ego_motion_loss = self.ego_motion_loss(pc1, pre_trans, gt_trans)
            motion_seg_loss = self.motion_seg_loss(mseg_pre, mseg_gt)
            dyn_flow_loss = self.dyn_flow_loss(pred_f, gt_f, dyn_mask)
            ...
            total_loss = self.w_self * self_sup_loss + self.w_em * ego_motion_loss +\
                         self.w_ms * motion_seg_loss + self.w_dyn * dyn_flow_loss 

        if args.model == 'cmflow_oc':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            ego_motion_loss = self.ego_motion_loss(pc1, pre_trans, gt_trans)
            motion_seg_loss = self.motion_seg_loss(mseg_pre, mseg_gt)
            pc1_warp = pc1 + pred_f
            ...
            items['opticalLoss'] = opt_flow_loss.item()
            total_loss = self.w_self * self_sup_loss + self.w_em * ego_motion_loss +\
                         self.w_ms * motion_seg_loss + self.w_opt * opt_flow_loss 

        if args.model == 'cmflow_olc' or args.model == 'cmflow_t':
            self_sup_loss, items = self.self_sup_loss(pc1, pc2, pred_f, vel1)
            ego_motion_loss = self.ego_motion_loss(pc1, pre_trans, gt_trans)
            motion_seg_loss = self.motion_seg_loss(mseg_pre, mseg_gt)
            dyn_flow_loss = self.dyn_flow_loss(pred_f, gt_f, dyn_mask)
