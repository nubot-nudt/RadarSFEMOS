...
DATA_PATH = '/home/toytiny/nuscenes/'
OUT_PATH_PC = '/home/toytiny/nuscenes/' + 'radar_pcs/'
IMG_PATH='/home/toytiny/nuscenes/'+'radar_img/'
SPLITS = {
          'mini_val': 'v1.0-mini',
...
HEIGHT_RANGE=(-3,5)
RES=0.15625
def get_radar_target(points,times):
    ...
    vel_x = points[6,:]
    vel_y = points[7,:]
    ## for original data, vel_x and vel_y is obtained by the decomposition of vel_r 
    theta=np.arctan2(y_points, (x_points+1e-4))
    vel_r = vel_x*np.cos(theta)+vel_y*np.sin(theta)
    times=times[0,:]   
    ...
    x_img = np.floor((x_points + FWD_RANGE[1])/RES)
    ...
    for split in SPLITS:
        ...
        for sample in nusc.sample:
          ...
          for sensor_name in RADAR_LIST:
              ...
              # aggregagte radar point cloud sweeps in lidar coordinate
              ...
              # Transpose radar point clound from lidar coordinate to car 
          ...
          for i in range(0,NUM_SWEEPS_RADAR): 
              cur_target=get_radar_target(radar_pcs[i],radar_times[i])
              for r_key in radar_target:
                  radar_target[r_key]=np.hstack((radar_target[r_key],cur_target[r_key]))
                 
          for r_key in radar_target:
              radar_target[r_key]=radar_target[r_key].tolist()
          # save the radar target to the file
          if len(radar_target['rcs'])<256:
              continue

          num_pcs+=1  
          BEV_show(radar_target,num_pcs,out_path_img)
          out_path_cur=out_path_pc+'/'+'radar_scenes-{}_pcs-{}.json'.format(num_scenes,num_pcs)
          ujson.dump(radar_target, open(out_path_cur, 'w'))
