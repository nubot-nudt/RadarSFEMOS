...
def point_ray_distance(warped_pcs, pixels, args):
    ...
    camera_projection_matrix = torch.from_numpy(args.camera_projection_matrix[:3,:3]).cuda().float().unsqueeze(0)
    cam_pcs = torch.inverse(camera_projection_matrix) @ pixels_h
