...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.layers = nn.ModuleList(nn.Conv1d(n, k, kernel_size=1, bias=False) for n, k in zip([input_dim] + h, h + [output_dim]))
...
class VoxSeT(nn.Module):
    def __init__(self):
        ...
        self.input_dim = 3#self.model_cfg.INPUT_DIM
        self.output_dim = 128#self.model_cfg.OUTPUT_DIM
        point_cloud_range = [0, -39.68, -3, 69.12, 39.68, 1]
        voxel_size = [0.32, 0.32, 4]
        grid_size = [0.32, 0.32, 4]
        self.input_embed = MLP(3, 16, self.input_dim, 2)
        ...
        self.post_mlp = nn.Sequential(
            nn.Conv1d(self.input_dim * 16, self.output_dim, kernel_size=1, bias=False),
        ...
        self.register_buffer('point_cloud_range', torch.FloatTensor(point_cloud_range).view(1, -1))#POINT_CLOUD_RANGE: [0, -39.68, -3, 69.12, 39.68, 1]
        self.register_buffer('voxel_size', torch.FloatTensor(voxel_size).view(1, -1))#VOXEL_SIZE: [0.32, 0.32, 4]
    ...
    def forward(self, points):
        ...
        points_offsets = points - self.point_cloud_range[:, :3]
        
        
        coords01x = points.clone()
        ...
        point_features = F.relu(src)
        point_coords = points[:, :4]
        
        pillar_features = F.relu(torch_scatter.scatter_max(src, inverse01x, dim=0)[0])
        voxel_coords = coords01x[:, [0, 3, 2, 1]]
...
class MLP_VSA_Layer(nn.Module):
    def __init__(self, dim, n_latents=8):
        ...
        self.score = nn.Conv1d(dim, n_latents, kernel_size=1, bias=False)
